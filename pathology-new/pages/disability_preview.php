<?php
    ...
    if(!empty($user_id)) {
        ...
        $user_details       = "SELECT * FROM disability_users_tmp WHERE id = ".trim($user_id);
        ...
        if(mysqli_num_rows($user_details_q)>0)
        {
            ...
            $user_details_arr['relation_type_name'] = $user_det_row['relation_type_name'];
...
?>
    ...
    <script>
        function do_submit(user_id) {
            $.post("<?php echo SITE_URL.'includes/common_functions.php?mode=confirm_submit' ?>", {user_id: user_id}, function(result){
                console.log(result);

                if(result == 1) {

                    $("#go_save_btn").text('Submitted. Print again');
                    $("#go_save_btn").attr('onclick', 'print_again()');

                    $("#go_back_btn").text('Go to lists');
                    $("#go_back_btn").attr('onclick', 'go_to_list_fnc()');
        ...
        function go_to_list_fnc() {
            
            window.location.href = '<?php echo SITE_URL.'index.php?pages=disability_list' ?>';
        }

        function print_again() {
            document.title='<?php echo 'KSDH-'.$user_details_arr['form_no'] ?> - Disability Certificate - <?php echo date('d-m-Y') ?>';
            window.print();
        }
    </script>
        ...
        <fieldset>
            <div class="col-sm-12 position_center">
                ...
                <div class="description_content">
                        ...
                        <div class="col-sm-9">
                            ...
                            <div class="third_sec">
                                ...
                                <div class="col-sm-8 padding-top-10 padding_left_0 padding_right_0">
                                    <span class="text-size-18">Years <?php echo (!empty($user_details_arr['relation_type']) && ($user_details_arr['relation_type'] != 'Other')) ? $user_details_arr['relation_type'].' of' : $user_details_arr['relation_type_name'].' of'; ?></span>
                    ...
                    <ul class="description_list text-size-18">
                        <li class"list_items>
                            <span>
                                ...
                                ?>
                                is a <?php echo $user_details_arr['disability_type'] ?>
                            </span>
                            <span>  
                                <?php 
                                ...
                                ?> person with (Nature of disability)
                            </span>
                        ...
                        <li>
                            <span>
                                ...
                                ?>
                                percentage of <?php echo $user_details_arr['disability_condition'];  ?> disability changes of variation is calculated as</span>
                    ...
                    <br><br><br><br>
                    <div class="stamp_area text-center" style="font-family: 'Merriweather Sans'; font-weight: 900;">
                        <p style="margin: 0;">Chairman of the Board</p>
                        <p style="margin: 0;">Kalna S.D. && S.S Hospital</p>
                        <p style="margin: 0;">Kalna, Purba Bardhman</p>
                        
                    </div>
                    <button style="margin: 10px auto auto; float: left;" id="go_save_btn" onclick="do_submit('<?php echo $user_id; ?>');" class="btn btn-lg btn-info print_hide">Submit & Print</button>		
                    <button style="margin: 10px auto auto; float: right;" class="btn btn-lg btn-info print_hide" id="go_back_btn" onclick="window.history.back();">Go back</button>		
                </div>
            </div>
        </fieldset>
    </div>
