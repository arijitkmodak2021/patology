...
(function($){
	...
	$.fn.selectstyle = function(option){
		...
		this.each(function(){
			...
			html = '<div class="selectstyle ss_dib '+setting.theme+'" style="">'+
				'<div id="select_style" class="ss_button" style="'+style+'">'+
				...
				'</div>';
			if(search == "true"){
				html += '<ul id="select_style_ul" sid="'+id+'" class="ss_ulsearch" style="max-height:'+setting.height+'px; width: 100%;"><div class="search" id="ss_search"><input id="p_search_name" name="p_search_name" type="text" placeholder="Search"></div><ul sid="'+id+'" style="max-height:'+(parseInt(setting.height) - 53)+'px; display: table; width: 100%;" class="ss_ul">'+html_op+'</ul></ul>';
			}
			else{
				html += '<ul id="select_style_ul" sid="'+id+'" style="max-height:'+setting.height+'px; display: table; width: 100%;" class="ss_ul">'+html_op+'</ul>';
			}
		...
		$("body").delegate(".p_clear_btn", "click", function(e) {
			console.log('Arijit');
			$('#select_style_text').html('Select a Patient');
			$('#p_search_name').val('');
			$('#search_patient_id').children('option').filter(function(){ }).prop('selected', false).change();
		});
